...
// Vercel KV kutubxonasini ulash (fallback bilan)
let kv = null;
try {
  const kvModule = await import('@vercel/kv');
  kv = kvModule.kv;
} catch (e) {
  console.log('[KV Warning] @vercel/kv modulini yuklashda fallback ishlatilmoqda');
}

// In-Memory store fallback (agar KV ulanmagan bo'lsa)
...
async function kvGet(key) {
  if (kv) {
    try {
      ...
      console.error(`[KV Get Error] Key ${key}:`, err.message);
    }
  }
  return global.__kv_store[key] ?? null;
}
...
async function kvSet(key, value) {
  if (kv) {
    try {
      ...
      console.error(`[KV Set Error] Key ${key}:`, err.message);
    }
  }
  global.__kv_store[key] = value;
}
...
export async function getSubscription(userId) {
  ...
  const data = await kvGet(`sub:${sUserId}`);
...
export async function getPayment(code) {
  if (!code) return null;
  return await kvGet(`payment:${code.trim().toUpperCase()}`);
}
...
export async function approvePayment(code, adminUserId) {
  ...
  await kvSet(`payment:${cleanCode}`, updatedPayment);
  ...
  await kvSet(`sub:${sUserId}`, updatedSub);
